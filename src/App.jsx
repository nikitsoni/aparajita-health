...
const MEDS = [
  ...
  { id:"purevise",  name:"Purevise 60k",generic:"Vitamin D3 60,000 IU",purpose:"Vitamin D",           slot:"afternoon", color:"#F7C948", note:"After lunch · Sunday",  doctor:"Dr. Sunaina Dubey", sundayOnly:true },
  // Night / Dinner
...
const maxPerfectStreak= h => {let max=0,cur=0;[...h].sort((a,b)=>a.date<b.date?-1:1).forEach(d=>{if(d.pct>=1){cur++;max=Math.max(max,cur);}else cur=0;});return max;};
const BADGES = [
  {id:"first",    icon:"🌱",name:"First Step",    desc:"Logged your first day",        check:(_,h)=>h.length>=1},
  ...
  {id:"pills50",  icon:"💊",name:"Pill Fifty",    desc:"50 pills taken",              check:(_,h)=>totalPills(h)>=50},
...
const getDailyQuote= () => { const day=Math.floor((new Date()-new Date("2024-01-01"))/864e5); return QUOTES[day%QUOTES.length]; };
const getBpStatus  = (s,d) => s<120&&d<80?{label:"Normal",color:"#4ECCA3"}:s<130&&d<80?{label:"Elevated",color:"#F7C948"}:s<140||d<90?{label:"Stage 1 High",color:"#F5A623"}:{label:"Stage 2 High",color:"#FF6B6B"};
const heatColor    = p => p===undefined||p===null?"rgba(255,255,255,0.04)":p===0?"rgba(255,107,107,0.25)":p<0.5?"rgba(245,166,35,0.3)":p<1?"rgba(180,142,255,0.4)":"rgba(78,204,163,0.65)";
...
.p1{left:7%;background:rgba(180,142,255,.5);animation-duration:14s;animation-delay:0s;}.p2{left:22%;background:rgba(255,156,194,.4);animation-duration:18s;animation-delay:4s;}.p3{left:40%;background:rgba(126,184,247,.45);animation-duration:11s;animation-delay:8s;}.p4{left:60%;background:rgba(180,142,255,.4);animation-duration:16s;animation-delay:2s;}.p5{left:77%;background:rgba(217,160,255,.5);animation-duration:12s;animation-delay:6s;}.p6{left:90%;background:rgba(255,156,194,.35);animation-duration:20s;animation-delay:1s;}.p7{left:50%;background:rgba(247,201,72,.3);animation-duration:15s;animation-delay:10s;}
...
.inp:focus{border-color:rgba(180,142,255,.5);}.inp::placeholder{color:var(--dim);}
...
.btn-export{background:rgba(78,204,163,.1);border:1px solid rgba(78,204,163,.25);border-radius:10px;color:var(--green);cursor:pointer;font-family:'DM Sans',sans-serif;font-size:.75rem;font-weight:600;padding:.6rem 1rem;transition:all .2s;display:flex;align-items:center;gap:.4rem;justify-content:center;width:100%;}
...
.badge-card{display:flex;flex-direction:column;align-items:center;gap:.3rem;padding:.8rem .5rem;border-radius:12px;border:1px solid;text-align:center;}
...
.heat-cell{width:100%;aspect-ratio:1;border-radius:4px;cursor:default;}
    ...
    <svg viewBox="0 0 100 100" className="ap-flower">
      {[0,72,144,216,288].map((a,i)=>(
        <g key={i} transform={`rotate(${a} 50 50)`}>
          <ellipse cx="50" cy="26" rx="9" ry="20" fill={i%2===0?"rgba(180,142,255,0.8)":"rgba(217,160,255,0.65)"} stroke="rgba(255,255,255,0.15)" strokeWidth="0.5"/>
        </g>
      ))}
      ...
      {[0,60,120,180,240,300].map((a,i)=>{const x=50+8.5*Math.cos(a*Math.PI/180),y=50+8.5*Math.sin(a*Math.PI/180);return<circle key={i} cx={x} cy={y} r="1.6" fill="#FFD97D"/>;})}
    </svg>
  );
}
...
function MedRow({m, taken, locked, pop, onToggle}){
  return(
    <div className={`med-row${(taken||locked)?" taken":""}`}
      style={locked?{opacity:.48,cursor:"default"}:{}}
      onClick={()=>!locked&&onToggle()}>
      <div className={`check${taken?" done":""}${pop?" pop":""}`} style={{borderColor:taken?"#4ECCA3":locked?"var(--dim)":m.color}}>
        {taken&&"✓"}
      </div>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:".9rem",fontWeight:600,color:(taken||locked)?"var(--dim)":"var(--text)",textDecoration:taken?"line-through":"none",display:"flex",alignItems:"center",gap:".4rem",flexWrap:"wrap"}}>
          {m.name}
          ...
          {m.duration&&<span style={{fontSize:".55rem",background:"rgba(180,142,255,.1)",color:"var(--purple)",border:"1px solid rgba(180,142,255,.25)",borderRadius:"8px",padding:".1rem .35rem",fontWeight:500}}>{m.duration}</span>}
        </div>
        <div style={{fontSize:".67rem",color:"var(--dim)",marginTop:".1rem"}}>
          {m.note} · <span style={{opacity:.7}}>{m.doctor}</span>
        </div>
...
export default function AparajitaHealth(){
  ...
  useEffect(()=>{
    (async()=>{
      setSync("loading");
      try{
        const[mRow,hist,bps]=await Promise.all([db.getMedLog(todayStr()),db.getMedHistory(120),db.getBpLogs(200)]);
        if(mRow?.log_data) setMedLog(mRow.log_data);
        setMedHist(hist); setBpLogs(bps); setSync("ok");
      }catch(e){
        setSync("err");
        try{const m=localStorage.getItem(`med:${todayStr()}`);const h=localStorage.getItem("med_hist");const b=localStorage.getItem("bp_logs");if(m)setMedLog(JSON.parse(m));if(h)setMedHist(JSON.parse(h));if(b)setBpLogs(JSON.parse(b));}catch(_){}
      }
      setLoading(false);
  ...
  useEffect(()=>{
    ...
    const iv=setInterval(()=>{
      ...
      if(h===13&&m===0) new Notification("☀️ Afternoon reminder",{body:"Tracnil M after lunch 🌸"});
  ...
  const syncToSupabase=useCallback((logData,taken,total,pct)=>{
    ...
    saveTimer.current=setTimeout(async()=>{
      ...
      try{
        ...
        setMedHist(prev=>[{date:today,taken,total,pct},...prev.filter(d=>d.date!==today)]);
        setSync("ok");
      }catch(e){setSync("err");localStorage.setItem(`med:${todayStr()}`,JSON.stringify(logData));}
    },800);
  },[]);

  const toggleMed=useCallback((id,countable)=>{
    const updated={...medLog,[id]:!medLog[id]};
    setMedLog(updated);
    setPopId(id); setTimeout(()=>setPopId(null),320);
    const taken=countable.filter(m=>updated[m.id]).length;
    const total=countable.length;
    syncToSupabase(updated,taken,total,total>0?taken/total:0);
  },[medLog,syncToSupabase]);

  const addBp=useCallback(async()=>{
    if(!bpForm.sys||!bpForm.dia) return;
    const entry={date:todayStr(),time_str:new Date().toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"}),systolic:parseInt(bpForm.sys),diastolic:parseInt(bpForm.dia),pulse:bpForm.pulse?parseInt(bpForm.pulse):null,notes:bpForm.notes||null};
    setBpLogs(prev=>[{...entry,id:Date.now()},...prev]);
    setBpForm({sys:"",dia:"",pulse:"",notes:""});
    setBpSaved(true); setTimeout(()=>setBpSaved(false),2200);
    db.insertBpLog(entry).catch(()=>{});
  },[bpForm]);

  const handleExport=useCallback(async()=>{
    setExporting(true);
    try{
      const{meds,bps}=await db.exportMonth(expYear,expMonth+1);
      if(meds.length) downloadCSV(`aparajita_meds_${MONTHS[expMonth]}_${expYear}.csv`,meds,["date","taken","total","pct","updated_at"]);
      if(bps.length) setTimeout(()=>downloadCSV(`aparajita_bp_${MONTHS[expMonth]}_${expYear}.csv`,bps,["date","time_str","systolic","diastolic","pulse","notes","created_at"]),300);
      if(!meds.length&&!bps.length) alert("No data for that month.");
    }catch(e){alert("Export failed.");}
    setExporting(false);
  },[expMonth,expYear]);

  // Derived
  const countable=MEDS.filter(m=>!m.sundayOnly||isSunday());
  ...
  const taken    =countable.filter(m=>medLog[m.id]).length;
  const total    =countable.length;
  ...
  for(let i=0;i<sortedHist.length;i++){
    const daysAgo=Math.round((new Date(todayStr())-new Date(sortedHist[i].date))/864e5);
    if(daysAgo===i&&sortedHist[i].pct>=1) streak++;
    else break;
  }
  const earnedBadges=BADGES.filter(b=>b.check(streak,medHist));

  const last7=Array.from({length:7},(_,i)=>{const d=new Date();d.setDate(d.getDate()-6+i);const ds=d.toISOString().split("T")[0];const rec=medHist.find(h=>h.date===ds);return{day:fmtDay(ds),pct:rec?Math.round(rec.pct*100):null,date:ds};});
  const heatDays=Array.from({length:35},(_,i)=>{const d=new Date();d.setDate(d.getDate()-34+i);const ds=d.toISOString().split("T")[0];const rec=medHist.find(h=>h.date===ds);return{date:ds,pct:rec?.pct};});
  const latest=bpLogs[0];
  const chartData=[...bpLogs].slice(0,20).reverse().map(b=>({date:b.displayDate||b.date?.slice(5),Systolic:b.systolic,Diastolic:b.diastolic}));
  ...
  if(loading) return(
    <div className="ap-root" style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",gap:"1rem"}}>
      <Flower/><div style={{fontFamily:"Cormorant Garamond,serif",fontSize:"1.4rem",color:"var(--muted)"}}>Loading…</div>
    </div>
  );
  ...
  return(
    <div className="ap-root">
      ...
      <nav className="ap-tabs">
        {[["today","🌸 Today"],["progress","🏆 Progress"],["bp","❤️ BP Log"],["bphist","📋 History"]].map(([k,l])=>(
      ...
      {tab==="today"&&(
        <div className="page">
          <div className={`sync ${sync}`}><span/>{sync==="ok"?"Synced to cloud ☁️":sync==="loading"?"Syncing…":"Offline — saved locally"}</div>

          {!notifOk&&(
            <button className="btn-notif" onClick={async()=>{if("Notification"in window){const p=await Notification.requestPermission();setNotifOk(p==="granted");}}}>
              <span style={{fontSize:"1.4rem"}}>🔔</span>
              <div><div style={{fontSize:".82rem",fontWeight:600,color:"#C4ADFF"}}>Enable Reminders</div><div style={{fontSize:".68rem",color:"var(--muted)"}}>8 AM · 1 PM · 10 PM</div></div>
              <span style={{marginLeft:"auto",fontSize:".75rem",color:"var(--purple)"}}>Allow →</span>
            </button>
          )}
          {notifOk&&<div style={{textAlign:"center",fontSize:".7rem",color:"var(--green)"}}>✓ Reminders active — 8 AM · 1 PM · 10 PM</div>}
          ...
          <div className="card" style={{padding:"1.4rem",display:"flex",alignItems:"center",gap:"1.4rem"}}>
            <svg width="90" height="90" viewBox="0 0 96 96" style={{flexShrink:0}}>
              ...
              <circle cx="48" cy="48" r="44" fill="none" stroke={pct===1?"#4ECCA3":"#B48EFF"} strokeWidth="6" strokeLinecap="round" strokeDasharray={C} strokeDashoffset={offset} transform="rotate(-90 48 48)" style={{transition:"stroke-dashoffset .6s ease"}}/>
            ...
            <div style={{flex:1}}>
              <div style={{fontFamily:"Cormorant Garamond,serif",fontSize:"1.4rem",color:"var(--text)",lineHeight:1.2}}>{pct===1?"All done today! 🎉":pct>.5?"Almost there 💜":"Good morning 🌸"}</div>
              <div style={{fontSize:".72rem",color:"var(--muted)",marginTop:".3rem"}}>{total-taken} remaining today</div>
              {streak>0&&<div style={{display:"inline-flex",alignItems:"center",gap:".35rem",marginTop:".5rem",background:"rgba(247,201,72,.1)",border:"1px solid rgba(247,201,72,.25)",borderRadius:"20px",padding:".2rem .65rem"}}><span>🔥</span><span style={{fontSize:".72rem",color:"#F7C948",fontWeight:600}}>{streak}-day streak</span></div>}
            </div>
          </div>
          ...
          <div className="card" style={{padding:"1.4rem 1.3rem",background:"linear-gradient(135deg,rgba(55,15,110,.35),rgba(180,142,255,.06))",borderColor:"rgba(180,142,255,.2)",textAlign:"center",position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:"-10px",left:"10px",fontFamily:"Cormorant Garamond,serif",fontSize:"6rem",color:"rgba(180,142,255,.1)",lineHeight:1,userSelect:"none"}}>"</div>
      ...
      {tab==="progress"&&(
        <div className="page">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:".65rem"}}>
            {[{label:"Streak",value:`${streak}🔥`,sub:"days"},{label:"Days Logged",value:medHist.length},{label:"Pills Taken",value:totalPills(medHist)}].map((s,i)=>(
          ...
          <div className="card" style={{padding:"1.2rem"}}>
            ...
            <ResponsiveContainer width="100%" height={130}>
              <BarChart data={last7} margin={{top:4,right:4,left:-28,bottom:0}}>
                ...
                <Tooltip content={({active,payload,label})=>active&&payload?.length?<div className="ct"><div className="ct-label">{label}</div><div style={{color:"#B48EFF"}}>{payload[0].value!=null?`${payload[0].value}% complete`:"No data"}</div></div>:null}/>
                <Bar dataKey="pct" radius={[6,6,0,0]}>
                  {last7.map((d,i)=><Cell key={i} fill={d.pct===null?"rgba(255,255,255,0.06)":d.pct===100?"#4ECCA3":d.pct>=50?"#B48EFF":"#F5A623"}/>)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="card" style={{padding:"1.2rem"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"4px",marginBottom:".7rem"}}>
              {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=><div key={d} style={{fontSize:".55rem",color:"var(--dim)",textAlign:"center"}}>{d}</div>)}
              {heatDays.map((d,i)=><div key={i} className="heat-cell" style={{background:heatColor(d.pct)}} title={`${d.date}: ${d.pct!=null?Math.round(d.pct*100)+"% complete":"no data"}`}/>)}
            </div>
            <div style={{display:"flex",gap:".6rem",justifyContent:"center",flexWrap:"wrap"}}>
              {[{c:heatColor(undefined),l:"No data"},{c:heatColor(0),l:"0%"},{c:heatColor(0.4),l:"<50%"},{c:heatColor(0.8),l:"<100%"},{c:heatColor(1),l:"Perfect"}].map((x,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:".25rem"}}>
                  <div style={{width:10,height:10,borderRadius:2,background:x.c}}/><span style={{fontSize:".58rem",color:"var(--dim)"}}>{x.l}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:".6rem"}}>
              {BADGES.map(b=>{const e=earnedBadges.find(x=>x.id===b.id);return(
                <div key={b.id} className={`badge-card ${e?"earned":"locked"}`}>
                  <span style={{fontSize:"1.6rem"}}>{b.icon}</span>
                  <div style={{fontSize:".68rem",fontWeight:600,color:e?"var(--purple)":"var(--dim)"}}>{b.name}</div>
          ...
          <div className="card" style={{padding:"1.2rem"}}>
            <div className="sec">📤 Monthly Export</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:".65rem",marginBottom:".8rem"}}>
              <div><label className="inp-label">Month</label>
                <select className="inp" value={expMonth} onChange={e=>setExpMonth(Number(e.target.value))}>
                  {MONTHS.map((m,i)=><option key={i} value={i}>{m}</option>)}
                </select></div>
              <div><label className="inp-label">Year</label>
                <select className="inp" value={expYear} onChange={e=>setExpYear(Number(e.target.value))}>
                  {[2024,2025,2026,2027].map(y=><option key={y} value={y}>{y}</option>)}
                </select></div>
            </div>
            <button className="btn-export" onClick={handleExport} disabled={exporting}>
              {exporting?"⏳ Exporting…":`⬇️ Download ${MONTHS[expMonth]} ${expYear} CSV`}
            </button>
            <div style={{fontSize:".62rem",color:"var(--dim)",marginTop:".5rem",textAlign:"center"}}>Meds log + BP readings as separate CSV files</div>
          </div>
        </div>
      )}
      ...
      {tab==="bp"&&(
        <div className="page">
          {latest&&(()=>{const s=getBpStatus(latest.systolic,latest.diastolic);return(
            <div className="card" style={{padding:"1.4rem",textAlign:"center"}}>
              ...
              <div style={{fontFamily:"Cormorant Garamond,serif",fontSize:"3.2rem",fontWeight:300,color:s.color,lineHeight:1}}>{latest.systolic}<span style={{fontSize:"1.4rem",color:"var(--dim)"}}>/{latest.diastolic}</span></div>
            ...
            <div className="card" style={{padding:"1.2rem"}}>
              ...
              <ResponsiveContainer width="100%" height={155}>
                <AreaChart data={chartData} margin={{top:4,right:4,left:-20,bottom:0}}>
                  <defs>
                    <linearGradient id="gS" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#B48EFF" stopOpacity={.35}/><stop offset="95%" stopColor="#B48EFF" stopOpacity={0}/></linearGradient>
                    <linearGradient id="gD" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#7EB8F7" stopOpacity={.3}/><stop offset="95%" stopColor="#7EB8F7" stopOpacity={0}/></linearGradient>
                  </defs>
                  ...
                  <Area type="monotone" dataKey="Systolic"  stroke="#B48EFF" strokeWidth={2} fill="url(#gS)" dot={{fill:"#B48EFF",r:3,strokeWidth:0}}/>
                  <Area type="monotone" dataKey="Diastolic" stroke="#7EB8F7" strokeWidth={2} fill="url(#gD)" dot={{fill:"#7EB8F7",r:3,strokeWidth:0}}/>
                </AreaChart>
              </ResponsiveContainer>
              <div style={{display:"flex",gap:"1rem",marginTop:".4rem",justifyContent:"center"}}>
                <span style={{fontSize:".65rem",color:"#B48EFF"}}>● Systolic</span><span style={{fontSize:".65rem",color:"#7EB8F7"}}>● Diastolic</span>
              </div>
            </div>
          )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:".65rem",marginBottom:".65rem"}}>
              {[{key:"sys",label:"Systolic *",ph:"120"},{key:"dia",label:"Diastolic *",ph:"80"},{key:"pulse",label:"Pulse (bpm)",ph:"72"},{key:"notes",label:"Notes",ph:"After rest…",type:"text"}].map(f=>(
                <div key={f.key} style={f.key==="notes"?{gridColumn:"span 2"}:{}}>
                  ...
                  <input className="inp" type={f.type||"number"} placeholder={f.ph} value={bpForm[f.key]} onChange={e=>setBpForm({...bpForm,[f.key]:e.target.value})} onKeyDown={e=>e.key==="Enter"&&addBp()}/>
                </div>
              ))}
            </div>
            <button className={`btn-primary${bpSaved?" success":""}`} onClick={addBp}>{bpSaved?"✓ Saved!":"Save Reading"}</button>
          </div>
          ...
          <div className="card" style={{padding:"1rem"}}>
            ...
            {[{l:"Normal",r:"< 120/80",c:"#4ECCA3"},{l:"Elevated",r:"120–129 / < 80",c:"#F7C948"},{l:"Stage 1 High",r:"130–139 / 80–89",c:"#F5A623"},{l:"Stage 2 High",r:"≥ 140 / ≥ 90",c:"#FF6B6B"}].map((t,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:".32rem 0",borderBottom:i<3?"1px solid rgba(255,255,255,.04)":"none"}}>
                <span style={{fontSize:".76rem",color:t.c}}>{t.l}</span><span style={{fontSize:".68rem",color:"var(--dim)"}}>{t.r}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab==="bphist"&&(
        <div className="page">
          ...
          {bpLogs.length===0?(
            <div style={{textAlign:"center",padding:"3rem 1rem",fontFamily:"Cormorant Garamond,serif",fontSize:"1.2rem",color:"var(--dim)"}}>No readings yet.<br/><span style={{fontSize:".8rem",fontFamily:"DM Sans"}}>Log your first BP reading 💜</span></div>
          ):bpLogs.map((bp,i)=>{const s=getBpStatus(bp.systolic,bp.diastolic);return(
            <div key={bp.id||i} className="card" style={{padding:".95rem 1rem",display:"flex",justifyContent:"space-between",alignItems:"center",gap:"1rem"}}>
              <div style={{flex:1}}>
                <div style={{fontFamily:"Cormorant Garamond,serif",fontSize:"1.7rem",color:s.color,lineHeight:1}}>{bp.systolic}<span style={{fontSize:".9rem",color:"var(--dim)"}}>/{bp.diastolic}</span></div>
                <div style={{fontSize:".62rem",color:"var(--dim)",marginTop:".18rem"}}>{bp.date} · {bp.time_str||bp.time}{bp.pulse?` · 💓 ${bp.pulse} bpm`:""}</div>
          ...
          {bpLogs.length>0&&<div style={{textAlign:"center",fontSize:".68rem",color:"var(--dim)",paddingBottom:".5rem"}}>{bpLogs.length} reading{bpLogs.length!==1?"s":""} · stored in Supabase ☁️</div>}
        </div>
      )}
